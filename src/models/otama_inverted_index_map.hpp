...
#ifndef OTAMA_INVERTED_INDEX_MAP_HPP
#define OTAMA_INVERTED_INDEX_MAP_HPP
...
namespace otama
{
	class InvertedIndexMap: public InvertedIndex
	{
	protected:
#ifdef _OPENMP
		...
		typedef std::unordered_map<uint32_t, VariableByteCodeVector> inverted_index_t;
#elsif OTAMA_WITH_UNORDERED_MAP_TR1
		typedef std::tr1::unordered_map<int64_t, metadata_record_t> metadata_t;
		typedef std::tr1::unordered_map<uint32_t, VariableByteCodeVector> inverted_index_t;
#else
		typedef std::map<int64_t, metadata_record_t> metadata_t;
		typedef std::map<uint32_t, VariableByteCodeVector> inverted_index_t;
#endif
	...
	public:
		InvertedIndexMap(otama_variant_t *options);
