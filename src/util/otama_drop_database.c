...
main(int argc, char **argv)
{
	...
	while ((opt = nv_getopt(argc, argv, "hdc:")) != -1){
		...
		case 'c':
			...
			if (config == NULL) {
				fprintf(stderr, "otama_drop_database: otama_yaml_read_file failed: %s: parse error or empty.\n", nv_getopt_optarg);
	...
	if (ret != OTAMA_STATUS_OK) {
		fprintf(stderr, "otama_drop_database: otama_open failed: %s\n", otama_status_message(ret));
		otama_variant_pool_free(&pool);		
		return -1;
	}
	...
	if (ret != OTAMA_STATUS_OK) {
		fprintf(stderr, "otama_drop_database: otama_drop_database failed: %s\n", otama_status_message(ret));
		otama_close(&otama);
		otama_variant_pool_free(&pool);		
		return -1;
	}
